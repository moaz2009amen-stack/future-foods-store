...
export default function ProductsClient({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (discountPrice != null && discountPrice >= salePrice) {
      setFormError("سعر الخصم لازم يكون أقل من سعر البيع");
      return;
    }
    ...
    if (error) {
      setFormError("تعذر حفظ المنتج: " + error.message);
      return;
    }
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <form onSubmit={handleSubmit} className="card p-6 w-full max-w-md flex flex-col gap-3 max-h-[90vh] overflow-y-auto">
            ...
            <div className="border-t border-border pt-3">
              <h3 className="text-xs font-bold text-muted mb-2">تفاصيل إضافية (اختيارية)</h3>
              <div className="flex flex-col gap-3">
                <input
                  placeholder="الوزن / الحجم (مثال: 500 جرام)"
                  className="w-full bg-surface2 border border-border rounded-lg px-3 py-2 text-sm"
                  value={form.weight}
