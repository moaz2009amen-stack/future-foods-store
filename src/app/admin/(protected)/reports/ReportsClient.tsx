...
export default function ReportsClient() {
  ...
  async function runReport() {
    ...
    const { data: orders } = await supabase
      .from("orders")
      .select("id, status, total, customer_phone, created_at")
    ...
    // نسبة تكرار الشراء: نسبة العملاء (برقم الهاتف) اللي عندهم أكتر من طلب في نفس الفترة
      ...
      {!loading && report && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
            ...
            <div className="card p-4">
              <div className="text-2xl font-extrabold text-accent">{report.avgOrderValue.toFixed(0)} ج.م</div>
              <div className="text-xs text-muted mt-1">متوسط قيمة الطلب</div>
            </div>
            <div className="card p-4">
              <div className="text-2xl font-extrabold text-accent">{report.repeatPurchaseRate.toFixed(0)}%</div>
              <div className="text-xs text-muted mt-1">نسبة تكرار الشراء</div>
            </div>
            <div className="card p-4">
