...
export default function StoreFooter({ settings }: { settings: StoreSettings }) {
  const socialLinks = [
    ...
    { url: settings.website_url, icon: MapPin, label: "الموقع" },
  ].filter((s) => s.url);
  ...
  return (
    <footer className="border-t border-border mt-16 bg-surface">
      <div className="max-w-7xl mx-auto px-4 py-10 grid gap-8 sm:grid-cols-3 text-sm">
        ...
        <div>
          <div className="font-bold mb-2">روابط مفيدة</div>
          <div className="flex flex-col gap-1 text-muted">
            <Link href="/about" className="hover:text-accent w-fit">من إحنا</Link>
            <Link href="/faq" className="hover:text-accent w-fit">الأسئلة الشائعة</Link>
            <Link href="/return-policy" className="hover:text-accent w-fit">سياسة الاسترجاع</Link>
            <Link href="/privacy-policy" className="hover:text-accent w-fit">سياسة الخصوصية</Link>
          </div>
        </div>
      </div>
